...
export default function SectionExamPage({
  ...
  const data = sections[params.section];
...
function Quiz({ data }: { data: SectionData }) {
            ...
            {q.options.map((opt) => {
              ...
              return (
                <button
                  ...
                  disabled={isAnswered}
                  className={`w-full text-left rounded-xl border-2 px-5 py-4 transition-all duration-150 flex items-start gap-4 ${style} ${
                ...
                >
                  ...
                  {isAnswered && opt.id === q.correctAnswer && (
                    <span className="ml-auto shrink-0 text-emerald-600 text-lg">✓</span>
                  )}
                  {isAnswered && opt.id === selected && opt.id !== q.correctAnswer && (
                    <span className="ml-auto shrink-0 text-red-500 text-lg">✗</span>
                  )}
                </button>
              );
            })}
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#f4f6fa]">
      <header className="bg-[#0c1f3f] shadow-lg">
        <div className="max-w-3xl mx-auto px-4 py-4 flex items-center gap-4">
          <Link href="/" className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center font-bold text-xs bg-[#d4a017] text-[#0c1f3f]">CP</div>
          </Link>
      ...
      <main className="flex-1 flex flex-col items-center py-10 px-4">
        <div className="w-full max-w-3xl">
          ...
          <div className={`rounded-2xl p-8 text-center mb-8 ${passed ? "bg-emerald-600" : "bg-[#0c1f3f]"}`}>
            ...
            <div className={`inline-flex items-center gap-2 px-5 py-2 rounded-full font-semibold text-sm ${
              passed ? "bg-white text-emerald-700" : "bg-[#d4a017] text-[#0c1f3f]"
            }`}>
          ...
          <div className="flex gap-3 mb-10">
            <button
              onClick={onRestart}
              className="flex-1 py-3.5 rounded-xl font-semibold text-sm bg-[#0c1f3f] text-white hover:bg-[#1a3560] transition-colors"
            >
              Volver a intentar
            </button>
            <Link
              href="/examenes-profesionales/cpa"
              className="flex-1 py-3.5 rounded-xl font-semibold text-sm border-2 border-[#0c1f3f] text-[#0c1f3f] text-center hover:bg-[#0c1f3f] hover:text-white transition-colors"
            >
              Otras secciones
            </Link>
          </div>
